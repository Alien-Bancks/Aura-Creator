document.addEventListener('DOMContentLoaded', () => {
    ...
    const tasks = {
        instagram: [{val:'legenda', text:'Legenda Viral'}, {val:'hashtags', text:'Hashtags'}, {val:'analise_visual', text:'Auditoria Feed'}],
        youtube: [{val:'titulo', text:'Títulos Clickbait'}, {val:'ideias', text:'Ideias de Vídeo'}],
        linkedin: [{val:'post', text:'Post Profissional'}, {val:'artigo', text:'Estrutura Artigo'}],
        twitter: [{val:'tweet', text:'Tweet'}, {val:'thread', text:'Thread'}],
    ...
    function updateTasks() {
        ...
        list.forEach(t => {
            ...
            opt.value = t.val; opt.textContent = t.text;
    ...
    generateBtn.addEventListener('click', async () => {
        ...
        document.querySelectorAll('.mockup-wrapper, .yt-grid, .generic-result').forEach(e => e.classList.add('hidden'));
    ...
    function exibirResultado(platform, task, text, imgUrl) {
        ...
        if(platform === 'instagram') {
            ...
            if(task === 'titulo') {
                ...
                let titles = text.split('\n').filter(l => l.trim().length > 5).map(l => l.replace(/^\d+[\.\)]\s*/, '').replace(/["*]/g, '').trim());
    ...
    async function loadHistory() {
        ...
        try {
            ...
            if (data.length === 0) {
                grid.innerHTML = '<p style="color:#666; padding:20px;">Nenhum histórico ainda.</p>';
                return;
            }
            ...
            data.forEach(item => {
                ...
                let cleanPath = item.image_path.replace('uploads/', '').replace('uploads\\', '');
                let webPath = `/uploads/${cleanPath}`;

                let imgHTML = item.platform === 'gerar_imagem' 
                ...
                div.onclick = () => {
                    ...
                    exibirResultado(item.platform, item.task_type, item.result_text, webPath);
                };
    ...
    btnVisual.onclick = () => { visualContainer.classList.remove('hidden'); rawTextArea.classList.add('hidden'); btnVisual.classList.add('active'); btnText.classList.remove('active'); };
    btnText.onclick = () => { visualContainer.classList.add('hidden'); rawTextArea.classList.remove('hidden'); btnText.classList.add('active'); btnVisual.classList.remove('active'); };
    btnCopy.onclick = () => { navigator.clipboard.writeText(lastGeneratedText); alert('Copiado!'); };
});